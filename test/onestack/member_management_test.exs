defmodule Onestack.MemberManagementTest do
  ...
  describe "member_credentials" do
    ...
    @invalid_attrs %{status: nil, product: nil, password: nil, job_id: nil, email: nil, hashed_password: nil, salt: nil}
    ...
    test "create_member_credentials/1 with valid data creates a member_credentials" do
      valid_attrs = %{status: "some status", product: "some product", password: "some password", job_id: "some job_id", email: "some email", hashed_password: "some hashed_password", salt: "some salt"}

      assert {:ok, %MemberCredentials{} = member_credentials} = MemberManagement.create_member_credentials(valid_attrs)
    ...
    test "create_member_credentials/1 with invalid data returns error changeset" do
      assert {:error, %Ecto.Changeset{}} = MemberManagement.create_member_credentials(@invalid_attrs)
    end

    test "update_member_credentials/2 with valid data updates the member_credentials" do
      member_credentials = member_credentials_fixture()
      update_attrs = %{status: "some updated status", product: "some updated product", password: "some updated password", job_id: "some updated job_id", email: "some updated email", hashed_password: "some updated hashed_password", salt: "some updated salt"}

      assert {:ok, %MemberCredentials{} = member_credentials} = MemberManagement.update_member_credentials(member_credentials, update_attrs)
    ...
    test "update_member_credentials/2 with invalid data returns error changeset" do
      member_credentials = member_credentials_fixture()
      assert {:error, %Ecto.Changeset{}} = MemberManagement.update_member_credentials(member_credentials, @invalid_attrs)
    ...
    test "delete_member_credentials/1 deletes the member_credentials" do
      member_credentials = member_credentials_fixture()
      assert {:ok, %MemberCredentials{}} = MemberManagement.delete_member_credentials(member_credentials)
      assert_raise Ecto.NoResultsError, fn -> MemberManagement.get_member_credentials!(member_credentials.id) end
    end
